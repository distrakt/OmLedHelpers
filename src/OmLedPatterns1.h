...
#define BALLS 30
    ...
    void innerTick(unsigned int ms, OmLed16Strip *strip) override
    {
        ...
        for (int ix = 0; ix < this->balls.size(); ix++)
        {
            ...
            strip->fillRange(oldX, newX + 1, b.co * bri);
...
#define _ptip2 (_ptip + _ptip - 1)
        ...
        float speed = getParamValueInt(3) / 100.0;
...
public:
    ...
    void innerTick(unsigned int ms, OmLed16Strip *strip)
    {
...
class Fire2 : public OmLed16Pattern
{
    ...
    {
        int flames = this->getParamValueInt(0);
        OmLed16 co = this->getParamValueColor(1);
    ...
    void innerTick(unsigned int ms, OmLed16Strip *strip) override
    {
        ...
        for(auto &f : this->flames)
        {
            ...
            if(f.huePhase > TAU)
                f.huePhase -= TAU;
            float b = mapRange(sin(f.phase), -1, 1, 0, 1);
            OmLed16 fCo;
...
public:
    ...
    void innerTick(unsigned int ms, OmLed16Strip *strip) override
    {
        ...
        for(int ic = 0; ic < k; ic++)
        {
            ...
            char ch = this->s[ic];
            ch -= 32;
            int bitLeft = 7 - ((font8x8Spans[ch] >> 4) & 0x0f);
            int bitRight = 7 - (font8x8Spans[ch] & 0x0f);
            int bitWidth = 0;
...
public:
    ...
    void innerInit() override
    {
        ...
        this->addColorParam("color", OmLed16(0, 0, 0));
        this->addIntParam("k", 0);
        this->addIntParam("k'", 0);
    ...
    void innerTick(unsigned int ms, OmLed16Strip *strip) override
    {
        ...
        int k = this->getParamValueInt(1) + this->getParamValueInt(2);
...
static void addOmLedPatterns1(OmLed16PatternManager &pm)
{
